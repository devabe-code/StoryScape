...
export default async function ReaderPage({
  ...
  const percent = book.progress > 0 ? book.progress : Math.min(100, Math.round((episodeNumber / book.episodes.length) * 100));
      ...
      <div className="border-b border-ink/10 bg-paper">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          ...
          <div className="min-w-0 text-center">
            ...
            <p className="text-xs text-ink/55">Episode {currentEpisode.number} of {book.episodes.length}</p>
          </div>
          <div className="flex items-center gap-1">
            <button className="focus-ring grid h-10 w-10 place-items-center rounded-md hover:bg-ink/5" type="button" aria-label="Reader settings">
              <Settings size={17} aria-hidden="true" />
            </button>
            <button className="focus-ring grid h-10 w-10 place-items-center rounded-md hover:bg-ink/5" type="button" aria-label="Episode menu">
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <article className="mx-auto w-full max-w-3xl">
          <header className="border-b border-ink/10 pb-6">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-ember">Episode {currentEpisode.number}</p>
            <h1 className="mt-3 font-serif text-4xl leading-tight sm:text-5xl">{currentEpisode.title}</h1>
          ...
          <div className="mt-8 space-y-7 font-serif text-[1.35rem] leading-9 text-ink sm:text-[1.45rem] sm:leading-10">
            ...
            <p>
              This demo uses a short adapted excerpt placeholder while the ingestion worker is still future work. The next milestone will replace these samples with sanitized public-domain chapter text and stored provenance.
            </p>
          </div>

          <nav className="mt-10 flex flex-col gap-3 border-t border-ink/10 pt-6 sm:flex-row sm:items-center sm:justify-between" aria-label="Episode navigation">
            <Link
        ...
        <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
          ...
          <section className="rounded-lg border border-ink/10 bg-white/65 p-4">
            ...
            <p className="mt-3 text-sm leading-6 text-ink/65">
              Mock persistence is active for the demo surface. Real user progress will move behind auth and the database schema next.
            </p>
          </section>
        </aside>
      </div>
